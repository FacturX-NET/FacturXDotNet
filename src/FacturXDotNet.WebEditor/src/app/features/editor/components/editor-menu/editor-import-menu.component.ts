...
@Component({
  ...
  template: `
    <li class="nav-item dropdown">
      ...
      <input #importPdfImageInput class="d-none" type="file" id="upload-file" name="upload-file" accept=".pdf" (change)="handlePdfImage($event)" />
    </li>
  `,
})
export class EditorImportMenuComponent {
  protected importPdfImageInput = viewChild<ElementRef>('importPdfImageInput');

  private editorStateService = inject(EditorStateService);

  importPdfImage() {
    this.importPdfImageInput()?.nativeElement?.click();
  }

  protected async handlePdfImage(_: Event) {
    const file: File | undefined = this.importPdfImageInput()?.nativeElement.files[0];
    if (file === undefined) {
      return;
    }
