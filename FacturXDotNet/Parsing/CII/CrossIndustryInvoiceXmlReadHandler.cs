...
struct CrossIndustryInvoiceXmlReadHandler(CrossIndustryInvoice result, ILogger? logger) : IXmlReadHandler
{
    readonly Stack<ReadOnlyMemory<char>> _pathStack = [];

    public bool FoundXmlDeclaration { get; private set; }
    public bool AtLeastOneTag { get; private set; }

    public void OnXmlDeclaration(ReadOnlySpan<char> version, ReadOnlySpan<char> encoding, ReadOnlySpan<char> standalone, int line, int column) => FoundXmlDeclaration = true;
    ...
    void HandleAttribute(ReadOnlyMemory<char> path, ReadOnlySpan<char> name, ReadOnlySpan<char> value)
    {
        switch (path.Span)
        {
            ...
            case
                ...
                {
                    result.SupplyChainTradeTransaction!.ApplicableHeaderTradeSettlement!.SpecifiedTradeSettlementHeaderMonetarySummation!.TaxTotalAmountCurrencyId =
                        value.ToString();
                }
                break;
